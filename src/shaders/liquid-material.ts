...
export class LiquidMaterialOwner {
  ...
  private projection(original: NodeMaterial['positionNode']): NonNullable<NodeMaterial['positionNode']> {
    // Keep the upstream morph/skinning/deformation graph as a real node edge,
    // not a closure capture inside Fn. All operands are bounded even when
    // disabled, and select returns the original position at amount zero.
    ...
    const packed = texture(this.fieldTexture(), uv).level(0);
  ...
  attachSurface(materials: SkinMaterials, skin: TextSkinEngine): void {
    ...
    // Each side keeps its own original normal. The back wall's side flip is
    // meaningful even when liquid mode is off.
    ...
    // Read the resolved normal. Using front.normalNode directly here makes
    // normalView re-enter setupNormal from the colour sub-build in Three r178.
    // That circular evaluation was responsible for the black head at amount 0.
