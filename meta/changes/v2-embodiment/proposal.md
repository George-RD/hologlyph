...
- Cloud viseme adapter (speech mode 2) exercised end to end: a hand-authored Amazon Polly speech-mark JSONL fixture (per res.viseme-provider-format) replayed through `SpeechEngine` into `MotionEngine`, asserting frame timing. No live provider account is used and no captured API response is committed.
